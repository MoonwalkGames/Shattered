...
void Mesh::scale(float scaleFactor)
{
	for (int i = 0; i < numFaces; i++)
	{
		faces[i].vertices[0].position.scale(scaleFactor);
		faces[i].vertices[1].position.scale(scaleFactor);
		faces[i].vertices[2].position.scale(scaleFactor);
	}
}
...
void Mesh::orderVertices()
{
	for (int i = 0; i < numFaces; i++)
	{
		Vec3 aToB = Vec3(faces[i].vertices[1].position.x - faces[i].vertices[0].position.x, faces[i].vertices[1].position.y - faces[i].vertices[0].position.y, 0);
		Vec3 bToC = Vec3(faces[i].vertices[2].position.x - faces[i].vertices[1].position.x, faces[i].vertices[2].position.y - faces[i].vertices[1].position.y, 0);
